...
`define ST_IDLE 0
   ...
   generate
  genvar gg;
  for (gg = 0; gg < g_num_breakpoints; gg = gg + 1) begin : gen_bp
    ...
    always @* begin
      ...
      // comparação dentro do range [lo..hi]
      break_hit[gg] =
        ((in_muxed[gg] >= break_compare_lo[gg]) &&
         (in_muxed[gg] <= break_compare_hi[gg])) ? in_valid[gg] : 1'b0;
    end
...
/*   always@(posedge clk_i)
	...
	if ( dbg_we_i ) begin
	   case (dbg_adr_i)
	     `DBG_REG_PC0: reload_pc_value[7:0] <= dbg_dat_i;
	   ...
	   case (dbg_adr_i)
	     `DBG_REG_PC0: dbg_dat_o <= x_pc_i[7:0];
 ...
 always @(posedge clk_i) begin
  if (rst_i) begin
    trigger_reload_pc <= 1'b0;
    trigger_halt      <= 1'b0;
    trigger_resume    <= 1'b0;
    reload_pc_value   <= 32'b0;
  end else begin
    trigger_halt      <= 1'b0;
    trigger_resume    <= 1'b0;
    trigger_reload_pc <= 1'b0;
    ...
    if (dbg_we_i) begin
      ...
      case (dbg_adr_i)
        ...
        `DBG_REG_PC2: dbg_dat_o <= x_pc_i[23:16];
        `DBG_REG_PC3: dbg_dat_o <= x_pc_i[31:24];
        default: dbg_dat_o <= 8'h00; // opcional
      endcase
    end
  end
end
